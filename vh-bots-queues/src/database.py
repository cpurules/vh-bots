...
class Database:
    def __init__(self, file='dbconfig.json'):
        ...
        if not self.db.hasCollection('queues'):
            self.db.createCollection(name='queues')
            self.db.reloadCollections()
        self.queues = self.db['queues']

        if not self.db.hasCollection('member-game-info'):
            self.db.createCollection(name='member-game-info')
            self.db.reloadCollections()
        self.member_game_info = self.db['member-game-info']
